...
const generateModuleDeclaration = (module, index, API) => {
  ...
  // Method Handler
  const genMethodString = (moduleMethod, parameters, includeType) => {
  ...
  const addMethod = (moduleMethod, prefix) => {
    ...
    moduleAPI.push(`${prefix}${moduleMethod.name}(${genMethodString(moduleMethod, moduleMethod.parameters, false)})${moduleMethod.name === 'constructor' ? '' : `: ${utils.typify(returnType)}`};`)
  }
  // Class constructor
  ...
  module.properties ? module.properties.sort((a, b) => a.name.localeCompare(b.name)).forEach(p => {
    moduleAPI.push(`${isStaticVersion ? 'static ' : ''}${p.name}${utils.isOptional(p) ? '?' : ''}: ${utils.typify(p.type)};`)
  }) : null
